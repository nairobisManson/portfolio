AOS.init();
/* Project Cards */

const projectcards = document.querySelector(".projectcards");

// Array of object for projects
const projects = [
  {
    title: "Quiz App",
    cardImage: "assets/images/project-page/mike.png",
    description: "A quiz app built using basic react.",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    className: "long",
  },
  {
    title: "Chess Engine",
    cardImage: "assets/images/project-page/pumkill.png",
    description: "A chess engine for a popular game dev engine.",
    tagimg: "https://image.flaticon.com/icons/png/512/643/643350.png",
    Previewlink: "",
    className: "big",
  },
  {
    title: "Flappy Bird",
    cardImage: "assets/images/project-page/silence.png",
    description: "Flappy bird game built using React.js",
    tagimg: "https://cdn.iconscout.com/icon/free/png-512/react-1-282599.png",
    Previewlink: "",
    className: "medium",
  },


  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/Infografia.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "long",
  },
  {
    title: "Recipe Ingredient Parser",
    cardImage: "assets/images/project-page/tib.png",
    description: "Recipe Ingredient Parser built using JavaScript.",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "",
    className: "medium",
  },

  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/instagram-flat-post-mockup.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "big",
  },
  {
    title: "Exercise Tracker",
    cardImage: "assets/images/project-page/facebook-post-mockup.png",
    description: "Exercise tracker built using basic redux.",
    tagimg: "https://miro.medium.com/max/2800/0*U2DmhXYumRyXH6X1.png",
    Previewlink: "",
    className: "big",
  },
  {
    title: "Poker Game",
    cardImage:
      "assets/images/project-page/business-card-mockup-In-binder-clip.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "big",
  },
  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/sushi.gif",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "long",
  },

  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/amy.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "long",
  },
  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/mansonlogo.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "medium",
  },

  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/taza.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "medium",
  },
  {
    title: "Poker Game",
    cardImage: "assets/images/project-page/loteriaapk.png",
    description: "Poker game built using typescript.",
    tagimg: "https://miro.medium.com/max/816/1*mn6bOs7s6Qbao15PMNRyOA.png",
    Previewlink: "",
    className: "big",
  },{
    title: "Recipe Ingredient Parser",
    cardImage: "assets/images/project-page/ilustration.png",
    description: "Recipe Ingredient Parser built using JavaScript.",
    tagimg:
      "data:image/png;base64,",
    Previewlink: "",
    className: "medium",
  },
];

// function for rendering project cards data
const showCards = () => {
  let output = "";
  projects.forEach(({ title, cardImage, Previewlink, className }) => {
    output += `       
          <div class="item ${className} style=" background: #dddcfa;" >
          <img class="img-ajust" src="${cardImage}" alt="">
          </div>
        `;
  });

  projectcards.innerHTML = output;
};
document.addEventListener("DOMContentLoaded", showCards);

function myFunction() {
  // Declare variables
  var input, button, i, skillcard, card, title;
  input = document.getElementById("myInput").value;
  input = input.toUpperCase();
  skillcard = document.getElementsByClassName("skill-card");
  card = document.getElementsByClassName("card");
  title = document.getElementsByClassName("title");

  // Loop through all list items, and hide those who don't match the search query
  for (i = 0; i < button.length; i++) {
    if (
      button[i].innerHTML.toUpperCase().includes(input) ||
      title[i].innerHTML.toUpperCase().includes(input)
    ) {
      skillcard[i].style.display = "";
      card[i].style.display = "";
    } else {
      skillcard[i].style.display = "none";
      card[i].style.display = "none";
    }
  }
}
